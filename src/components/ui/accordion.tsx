...
>(({ className, children, ...props }, ref) => (
  <AccordionPrimitive.Header className="flex">
    ...
    >
      {children}
      <ChevronDown className="size-4 shrink-0 text-(--color-foreground-muted) transition-transform duration-(--duration-normal)" />
    </AccordionPrimitive.Trigger>
...
>(({ className, children, ...props }, ref) => (
  <AccordionPrimitive.Content
    ...
    {...props}
  >
    <div className={cn("pb-4 text-(--color-foreground-muted)", className)}>{children}</div>
  </AccordionPrimitive.Content>
));
